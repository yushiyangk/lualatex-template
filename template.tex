...
\usepackage{titling}  % Enable document title layout adjustments
	\thanksmarkseries{alph} \continuousmarks  % Subsequent footnotes after title continues the alphabet
\usepackage{authblk}  % Provide author affiliations formatting
	\renewcommand\Authands{, }  % Do not add "and" between final two author names, when there are more than two authors
	\let\oldthanks\thanks \renewcommand\thanks[1]{\begingroup\let\rlap\relax\oldthanks{#1}\endgroup} % Fix stupid rlap of footnote marks in authors list
%% Optional
...
\usepackage{siunitx}[=v2]  % Provide formatted numerals and unit symbols, and S table columns for aligning numerals of varying digit lengths
	\sisetup{
...
%%% Tables and figures
\usepackage{float}  % Enable float layout adjustments and custom floats (consider using newfloat intead), and provide the strong place-here flag [H]
...
\usepackage{accsupp}  % Accessibility support; allows modifying selectable text (load after hyperref)
%% Optional
...
\usepackage{embedfile}  % Enable embedding files in the PDF output
